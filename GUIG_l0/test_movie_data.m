...
opts.eta = 0.5;               %  rate = 0.7 eta = 0.06; rate = 0.5, eta = 0.08; rate = 0.8, eta = 0.06; For dataset = 2;
opts.lambda = 2.0*norm(data_train,2);            % 1 for lp
opts.mu = 0.25*opts.lambda;
opts.p_i = p_i;
opts.p = p_i(1);
...
RMSE = MatCompRMSE(X, Y', opts.I_test, opts.J_test, opts.data_test);
% RMSE = CompRMSEm(X*Y,opts.I_test+ m*(opts.J_test-1), opts.data_test);
time = toc(t0);
rate=num2str(train_rate);
%% save parameter and results
para_name = [Para_fold,'_','GUIG_l0_',rate,'_',fname,'.mat'];
save(['D:\贾西西\2017工作\非凸低秩矩阵分解方法\低秩矩阵分解代码\GUIG_real_MC\Para_setting\',para_name],'opts');
record_name = ['Time_RMSE_GUIG_l0_',rate,'_',fname,'.mat'];
